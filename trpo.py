...
def choice_tariff():
    ...
    for tariff_name, tariff_details in tariff_plans.items():
        if tariff_details["operator"] == operator:
            ...
            if tariff_minutes >= int(minutes.split()[0]) and tariff_messages >= int(messages.split()[0]) and tariff_internet >= int(internet.split()[0]) and tariff_cost < optimal_tariff_cost:
    ...
    if optimal_tariff:
        ...
        choice = input("Хотите подключить данный тариф? (да/нет): ")
        if choice.lower() == "да":
            print("Тариф успешно подключен.")
        elif choice.lower() == "нет":
            print("Тариф не будет подключен.")
        else:
            print("Некорректный выбор. Тариф не будет подключен.")
...
def print_tariff_plans():
    ...
    for tariff_name, tariff_details in tariff_plans.items():
        ...
        print(f"Интернет-трафик: {tariff_details['internet']}")
        print("======================")
